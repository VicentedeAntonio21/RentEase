        ...
        <div class="bg-white dark:bg-[#252B3E] rounded-xl shadow-sm border border-gray-100 dark:border-white/5 p-5">
            <form method="GET" action="{{ route('listings.index') }}" class="grid grid-cols-2 md:grid-cols-6 gap-3">
                ...
                <div>
                    ...
                    <select name="property_type"
                        ...
                        @foreach (['apartment', 'house', 'condo', 'room', 'other'] as $type)
                            <option value="{{ $type }}" @selected(request('property_type') === $type)>{{ ucfirst($type) }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex items-end gap-2">
                    <button type="submit"
                        class="flex-1 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-dark flex items-center justify-center gap-1">
                        <i class="ri-search-line"></i> Search
                    </button>
                    @if (request()->anyFilled(['city', 'min_price', 'max_price', 'bedrooms', 'property_type']))
                        <a href="{{ route('home') }}"
                            class="px-3 py-2 border border-gray-200 dark:border-white/10 rounded-lg text-sm text-gray-500 dark:text-gray-400">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            @empty
                <div class="col-span-full text-center py-16">
                    ...
                    <a href="{{ route('listings.index') }}" class="inline-block mt-2 text-primary text-sm font-medium">Clear
                        filters</a>
                </div>
            @endforelse
        </div>
