<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        @if ($applications->isEmpty())
            <div
                class="bg-white dark:bg-[#252B3E] rounded-xl shadow-sm border border-gray-100 dark:border-white/5 p-12 text-center">
        ...
        @else
            <div
                class="bg-white dark:bg-[#252B3E] rounded-xl shadow-sm border border-gray-100 dark:border-white/5 divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <div class="flex items-center gap-3 shrink-0">
                            ...
                            @if ($application->status === 'pending')
                                <button type="button" x-data=""
                                    x-on:click="$dispatch('open-modal', 'cancel-application-{{ $application->id }}')"
                                    class="px-3 py-1.5 text-xs font-medium border border-gray-200 dark:border-white/10 rounded-lg text-gray-500 dark:text-gray-400 hover:border-danger hover:text-danger">
                                    Cancel
                                </button>

                                <x-confirm-modal name="cancel-application-{{ $application->id }}" title="Cancel this application?"
                                    message="You can re-apply for this unit later if you change your mind."
