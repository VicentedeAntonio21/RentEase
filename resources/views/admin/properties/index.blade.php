            ...
            @foreach ($properties as $property)
                <div class="p-5 flex items-center justify-between gap-4">
                    ...
                    <button type="button" x-data=""
                        x-on:click="$dispatch('open-modal', 'admin-delete-property-{{ $property->id }}')"
                        class="px-3 py-1.5 text-sm font-medium border border-danger/20 rounded-lg text-danger hover:bg-danger/10 shrink-0">
                        Remove
                    </button>

                    <x-confirm-modal name="admin-delete-property-{{ $property->id }}" title="Remove this property?"
                        message="This will permanently remove '{{ $property->title }}' along with all its units and applications."
