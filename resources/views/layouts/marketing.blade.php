...
<body class="font-sans antialiased bg-white dark:bg-[#1E2235] text-gray-800 dark:text-gray-200">
    ...
    <nav x-data="{ open: false }" class="border-b border-gray-100 dark:border-white/5">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <a href="{{ auth()->check() ? route('dashboard') : route('home') }}" class="flex items-center gap-2">
                <img src="{{ asset('images/rentease-logo-full.png') }}" alt="RentEase" class="h-9 w-auto">
            </a>

            <div class="hidden md:flex items-center gap-8">
                <a href="{{ route('listings.index') }}"
                    class="text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-primary">Browse Listings</a>
                <a href="#how-it-works"
                    class="text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-primary">How It Works</a>
            </div>
            ...
            <div class="hidden md:flex items-center gap-4">
                ...
                @else
                    <a href="{{ route('login') }}"
                        class="text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-primary">Log in</a>
        ...
        <div x-show="open" x-cloak class="md:hidden border-t border-gray-100 dark:border-white/5 px-4 py-4 space-y-3">
            <a href="{{ route('listings.index') }}"
                class="block text-sm font-medium text-gray-600 dark:text-gray-300">Browse Listings</a>
    ...
    <footer class="border-t border-gray-100 dark:border-white/5 mt-20">
        <div
            class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
